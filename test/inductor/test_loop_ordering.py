...
class TestSplitIterationRanges(MockSchedulerTest):
    ...
    def test_is_compatible_exact_match(self):
        """is_compatible returns True when groups match lengths."""
        from torch._inductor.codegen.simd import SIMDKernel

        result = SIMDKernel.is_compatible(
            [sympy.Integer(4), sympy.Integer(8)],
            [[sympy.Integer(4)], [sympy.Integer(8)]],
        )
        self.assertTrue(result)

    def test_is_compatible_groups_exhausted(self):
        """is_compatible returns False when groups are exhausted."""
        from torch._inductor.codegen.simd import SIMDKernel

        # groups=[1, 2, 2] can only absorb 2 sizes of 2,
        # leaving the third size=2 with no group.
        result = SIMDKernel.is_compatible(
            [sympy.Integer(1), sympy.Integer(2), sympy.Integer(2)],
            [[], [sympy.Integer(2), sympy.Integer(2), sympy.Integer(2)]],
        )
